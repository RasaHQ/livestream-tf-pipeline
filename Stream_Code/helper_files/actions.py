...
class ActionMeetup(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		response = """There is a {} in {}. They are having an event called {} on {}. Would you like to join the meetup?""".format(meetup_name, location, next_event_name, next_event_time1)
...
class ActionJoinEvent(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		try:	
			...
			dispatcher.utter_message('All done. I have booked you a spot at {} which is happening at {}'.format(event_name, event_time))
...
class ActionSuggestRoute(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		try:
			...
			dispatcher.utter_message('No problem. Here are the instructions of how to get to the venue: {}'. format(text_response))
